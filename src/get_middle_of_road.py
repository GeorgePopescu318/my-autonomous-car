...
def get_middle(frame,displayVideo):
    ...
    if middleBirdEye != []:
        ...
        trueMiddle = cv2.perspectiveTransform(middleBirdEye, invMatrix)
        trueMiddle = trueMiddle.astype(int)
    else:
        trueMiddle = []

    
    if displayVideo and len(trueMiddle) != 0:
        for i in range(len(trueMiddle)-1):
            cv2.line(frame, (int(trueMiddle[i][0][0]),int(trueMiddle[i][0][1])),(int(trueMiddle[i+1][0][0]),int(trueMiddle[i+1][0][1])), (0,0,255), 2)
