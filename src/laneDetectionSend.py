...
def processImage(inpImage):
    ...
    #HLS stands for hue light saturation, another color space for image processing
    #TODO why is hls not used?
    #R: Looks like if I just use hls instead of rgb things dont go as planned
    ...
    #Makes a gray version of the masked image
    gray = cv2.cvtColor(hls_result, cv2.COLOR_BGR2GRAY)

    #if 160 < px < 255 then px = 255 else px = 0 so that the lane is white

    blur = cv2.GaussianBlur(gray,(3, 3), 0)

    ret, thresh = cv2.threshold(blur, 190, 255, cv2.THRESH_BINARY)

    #applies a Gaussina Blur kernel of 3x3 to reduce the noise by smothing the image
    

    #applies Canny edge detection with 40 for strong edges and 60 for weak
...
def viewPerspectiveWarp(inpImage):
    ...
    #src =  [[80, 70],[0, 180],[320, 180],[230, 70]]
    # print(img_size)
    # # Window to be shown
...
def perspectiveWarp(inpImage):
    ...
    #BRAIN values for warp
    #TODO use these points
    # Perspective points to be warped
    ...
    #TEST Values
    # Perspective points to be warped
    #src = np.float32([[80, 70],[0, 180],[320, 180],[230, 70]])
...
################################################################################
#### START - FUNCTION TO PLOT THE HISTOGRAM OF WARPED IMAGE (BIRDEYEVIEW IMAGE) ####################
def plotHistogram(inpImage):
    #TODO understand this better
    #R: adds the values from the photo making a 1D array with the sum from each column so that we can identify the lane
    histogram = np.sum(inpImage[inpImage.shape[0] // 2:, :], axis = 0)

    midpoint = int(histogram.shape[0] / 2)
    #argmax of :midpoint and midpoint: represents the point where there is the most white
    leftxBase = np.argmax(histogram[:midpoint])
    rightxBase = np.argmax(histogram[midpoint:]) + midpoint #TODOwhy + midpoint?
...
def slide_window_search(binary_warped, histogram):
    ...
    #peaks of the histogram for left and right
    leftx_base = np.argmax(histogram[:midpoint])
    rightx_base = np.argmax(histogram[midpoint:]) + midpoint #TODOwhy + midpoint?
    ...
    #the center of the first left and right rectangle
    ...
    #min no of pixels needed for the rectangle to recenter
    minpix = 50

    #centers of the left and right rectangles
    left_lane_inds = []
    right_lane_inds = []

    #### START - Loop to iterate through windows and search for lane lines #####
    for window in range(nwindows):
        #finds the coordinates of the rectangles
        ...
        #finds the number of white pixels in the rectangle
        ...
        #update the position of the next rectangle
        #TODO why the next rectanle and not this one?
    ...
    #TODO implement these plt sizes
    plt.xlim(0, 480)
    plt.ylim(360, 0)
...
def addText(img, radius, direction, deviation, devDirection):
    ...
    cv2.putText(img, text , (50,100), font, 0.5, (0,100, 200), 2, cv2.LINE_AA)
    cv2.putText(img, text1, (50,150), font, 0.5, (0,100, 200), 2, cv2.LINE_AA)
